...
namespace CheatOnYourDayOnes.Vehicles
{
    ...
    public sealed class DriveableCar : MonoBehaviour
    {
        ...
        public bool IsOccupied=>_occupied;public Vector3 DriveVelocity=>transform.forward*_driveSpeed;public float SpeedKmh=>Mathf.Abs(_driveSpeed)*3.6f;
        ...
        private void FixedUpdate(){if(!_occupied)return;if(_brake)_driveSpeed=Mathf.MoveTowards(_driveSpeed,0f,brakeAcceleration*Time.fixedDeltaTime);else if(_throttle>.01f)_driveSpeed=Mathf.MoveTowards(_driveSpeed,topSpeed,forwardAcceleration*Mathf.Max(.18f,_throttle)*Time.fixedDeltaTime);else if(_throttle<-.01f)_driveSpeed=Mathf.MoveTowards(_driveSpeed,-reverseTopSpeed,reverseAcceleration*Mathf.Max(.18f,-_throttle)*Time.fixedDeltaTime);else _driveSpeed=Mathf.MoveTowards(_driveSpeed,0f,(engineBraking+rollingResistance*Mathf.Abs(_driveSpeed))*Time.fixedDeltaTime);Vector3 local=transform.InverseTransformDirection(_rb.linearVelocity);float speed01=Mathf.Clamp01(Mathf.Abs(_driveSpeed)/topSpeed),grip=Mathf.Lerp(lateralGripLowSpeed,lateralGripHighSpeed,speed01);local.x=Mathf.MoveTowards(local.x,0f,grip*Time.fixedDeltaTime);local.z=_driveSpeed;Vector3 wanted=transform.TransformDirection(new Vector3(local.x,0f,local.z));wanted.y=_rb.linearVelocity.y;_rb.linearVelocity=wanted;float authority=Mathf.Clamp01(Mathf.Abs(_driveSpeed)/1.7f),steerMult=Mathf.Lerp(1f,highSpeedSteerFactor,speed01),reverse=_driveSpeed<-.05f?-1f:1f,targetYawRateDeg=_steer*steeringRate*steerMult*authority*reverse;_yawRate=Mathf.MoveTowards(_yawRate,targetYawRateDeg,steeringYawAcceleration*Time.fixedDeltaTime);if(Mathf.Abs(_steer)<.01f)_yawRate=Mathf.MoveTowards(_yawRate,0f,steeringYawAcceleration*1.35f*Time.fixedDeltaTime);Vector3 av=_rb.angularVelocity;av.y=_yawRate*Mathf.Deg2Rad;av.x*=.82f;av.z*=.82f;_rb.angularVelocity=av;DetectNPCHits();DetectNPCOverrun();_debugTimer+=Time.fixedDeltaTime;if(_debugTimer>=1f&&Mathf.Abs(_rawThrottle)>.1f){_debugTimer=0;Debug.Log($"[CYDOY] CAR INPUT={_rawThrottle:F0} throttle={_throttle:F2} commanded={_driveSpeed:F2}m/s actual={transform.InverseTransformDirection(_rb.linearVelocity).z:F2}m/s yawRate={_yawRate:F1}",this);}}

        private void OnCollisionEnter(Collision collision){HandleSolidObstacleCollision(collision);}
        private void OnCollisionStay(Collision collision){HandleSolidObstacleCollision(collision);}
