...
namespace CheatOnYourDayOnes.Vehicles
{
    ...
    public sealed class DriveableCar : MonoBehaviour
    {
        ...
        [SerializeField] private float interactionDistance=3.5f,tyreGroundClearance=.015f,npcHitPadding=.85f,rollingResistance=.18f;
        ...
        private readonly List<Renderer> _wheelRenderers=new();private readonly List<SphereCollider> _wheelSupportColliders=new();private readonly HashSet<NPCWanderer> _npcHitThisContact=new();
        ...
        private void ConfigureRigidbody(){_rb.mass=1350;_rb.useGravity=true;_rb.isKinematic=false;_rb.constraints=RigidbodyConstraints.None;_rb.linearDamping=.03f;_rb.angularDamping=2.6f;_rb.interpolation=RigidbodyInterpolation.Interpolate;_rb.collisionDetectionMode=CollisionDetectionMode.ContinuousDynamic;_rb.maxAngularVelocity=3;_rb.centerOfMass=centerOfMass!=null?transform.InverseTransformPoint(centerOfMass.position):new Vector3(0,-.42f*_modelScale,.08f*_modelScale);}
        ...
        private void FixedUpdate(){if(!_occupied)return;
            if(_brake)_driveSpeed=Mathf.MoveTowards(_driveSpeed,0f,brakeAcceleration*Time.fixedDeltaTime);else if(_throttle>.01f)_driveSpeed=Mathf.MoveTowards(_driveSpeed,topSpeed,forwardAcceleration*Mathf.Max(.18f,_throttle)*Time.fixedDeltaTime);else if(_throttle<-.01f)_driveSpeed=Mathf.MoveTowards(_driveSpeed,-reverseTopSpeed,reverseAcceleration*Mathf.Max(.18f,-_throttle)*Time.fixedDeltaTime);else _driveSpeed=Mathf.MoveTowards(_driveSpeed,0f,(engineBraking+rollingResistance*Mathf.Abs(_driveSpeed))*Time.fixedDeltaTime);
            Vector3 local=transform.InverseTransformDirection(_rb.linearVelocity);float speed01=Mathf.Clamp01(Mathf.Abs(_driveSpeed)/topSpeed),grip=Mathf.Lerp(lateralGripLowSpeed,lateralGripHighSpeed,speed01);local.x=Mathf.MoveTowards(local.x,0f,grip*Time.fixedDeltaTime);local.z=_driveSpeed;Vector3 wanted=transform.TransformDirection(new Vector3(local.x,0f,local.z));wanted.y=_rb.linearVelocity.y;_rb.linearVelocity=wanted;
            ...
            DetectNPCHits();_debugTimer+=Time.fixedDeltaTime;if(_debugTimer>=1f&&Mathf.Abs(_rawThrottle)>.1f){_debugTimer=0;Debug.Log($"[CYDOY] CAR INPUT={_rawThrottle:F0} throttle={_throttle:F2} commanded={_driveSpeed:F2}m/s actual={transform.InverseTransformDirection(_rb.linearVelocity).z:F2}m/s yawRate={_yawRate:F1}",this);}}
        private void DetectNPCHits(){Bounds b=_chassisCollider!=null?_chassisCollider.bounds:new Bounds(transform.position,new Vector3(2,1.5f,4));Vector3 probeCenter=b.center+transform.forward*(b.extents.z+npcHitPadding*.65f);Vector3 half=new Vector3(b.extents.x+npcHitPadding,b.extents.y+npcHitPadding*.5f,npcHitPadding+1.1f*_modelScale);Collider[] hits=Physics.OverlapBox(probeCenter,half,transform.rotation,~0,QueryTriggerInteraction.Collide);HashSet<NPCWanderer> current=new();foreach(Collider hit in hits){if(hit==null||hit.transform.IsChildOf(transform))continue;NPCWanderer npc=hit.GetComponentInParent<NPCWanderer>();if(npc==null)continue;current.Add(npc);if(_npcHitThisContact.Contains(npc))continue;float kmh=SpeedKmh;if(npc.HitByVehicle(DriveVelocity,transform.position)){ApplyNPCImpactSpeedResponse(kmh);_npcHitThisContact.Add(npc);}}_npcHitThisContact.RemoveWhere(n=>!current.Contains(n));}
